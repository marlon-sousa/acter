...
3. **Port** — a trait seam, in either direction. **Driven ports** define what the
   ...
   TerminalEngine, Clock, EventSink) — adapters implement them. **Driving ports**
...
- Known upcoming ports this rule forces: `Clock` (anything time-based) and
  `EventSink` (event emission to the frontend) — tempting to inline, world-touching,
  therefore ported. The **beep is not a backend port**: it is a frontend view adapter
  (`ui/src/adapters/beep.ts`, WebAudio, behind `ports/beep_view.ts`), rendering the
  too-big verdict audibly exactly as the live region renders it in speech, so all
  audible output lives in one layer (spec A3, decision 1). A genuinely backend-side
...
- src/ports/: one interface per file — backend_api.ts, edit_field_view.ts,
  buffer_view.ts, announcer_view.ts, beep_view.ts
- src/protocol.ts — generated, committed: the TypeScript mirror of acter-core's IPC
  types (types only, no imports; regenerated by the acter-app binding test). Do not
  edit by hand.
- src/routers/: tauri.ts (the only module importing @tauri-apps/api)
- src/controllers/: app.ts
- src/adapters/: edit_field.ts, buffer.ts, announcer.ts, keyboard.ts, beep.ts (the
  decided home of the completion beep — WebAudio, behind beep_view.ts)
...
serde types in `acter-core`, deriving `specta::Type`. TypeScript definitions are
generated from them with the specta/tauri-specta type exporter
(`specta-typescript` + `specta-serde`, which applies the serde attributes), so the
protocol lives in exactly one place and the frontend cannot silently drift. The
export is **types only** — no invoke/channel runtime helpers, since the frontend
router hand-writes typed invoke wrappers (see "The two directions") — so the
generated module (`ui/src/protocol.ts`) imports nothing and `routers/tauri.ts` stays
the sole importer of `@tauri-apps/api`. The generator lives in a test
(`crates/acter-app/tests/protocol_bindings.rs`) that writes the committed binding; CI
regenerates and fails on any diff (spec A2).
